...
public class ZooKeeper {
   ...
   public static void main(String[] args) throws FileNotFoundException {
      ...
      Scanner line = new Scanner(new File("ZooAnimals.txt"));        //this has all the lines of the file and will go through them
      String answer = null;
      
      Tree animals = new Tree("Does it have fur?", new Tree("bunny"), new Tree("Lizard"));         //basic tree
      Tree current = animals;                                                                      //animals is the tree that gets built off of. Current is the place we are in that tree
      ...
      while(line.hasNext()) {                                     //this adds all the things that were saved from previous trees
         ...
         if(colon1 != 0 && colon2 != 0) {                         //if it doesn't have two colons it is not going to read the line and it will skip it
            
            animals = master.add(curLine.substring(0,colon1), curLine.substring(colon1+1,colon2), curLine.substring(colon2+1, curLine.length()), animals);
         }
         colon1 = 0;                   //this allows me to write notes if I wanted on the file and as long as there isn't two colons it won't cause any problems
         colon2 = 0;
      ...
      Scanner outputLine = new Scanner(new File("output.txt"));            //I use output to save all the data for zoo animals so I can copy it over change it and then paste it back into zoo animals
      
      while(outputLine.hasNext()) {
         mainFile.println(outputLine.nextLine());                 //this saves all the work into ZooAnimals setting up adding the next few lines
      }
   ...
   public static Tree addAnimal(Tree animals, String code, ZooMaster master, PrintStream mainFile) {    //this allows new animals to be added
   
      Scanner sc = new Scanner(System.in);
      System.out.println("Do you want to add your animal?");                                             //added this in the method so if I wanted to use it elsewhere I wouldn't have to ask them this again
      if(sc.nextLine().equals("yes")) {
               
         System.out.println("what animal were you thinking of?");
         String newAnimal = sc.nextLine();
         System.out.println("and what would be a good question to distinguish this animal? (comparing to the animal you said it wasn't)");
         String newQuestion = sc.nextLine();                                                             //these are all questions for the new animal to have a way to identify it
         System.out.println("for the question: (" + newQuestion + ") was the " + newAnimal + " a yes or a no to that question?");
         String yesOrNo = sc.nextLine();
         System.out.println("Adding... press enter to continue");
         
         animals = master.add(code + yesOrNo.substring(0,1), newQuestion, newAnimal, animals);           //adds the new question and animal into the main tree
         mainFile.println(code + yesOrNo.substring(0,1) + ":" + newQuestion + ":" + newAnimal);          //adds the animal to the main file so you can reload the main tree
      }
